...
-- MAGIC <img width="1px" src="https://ppxrzfxige.execute-api.us-west-2.amazonaws.com/v1/analytics?category=lakehouse&org_id=3094482429007608&notebook=%2F01-Data-ingestion%2F01.1-DLT-fraud-detection-SQL&demo_name=lakehouse-fsi-fraud&event=VIEW&path=%2F_dbdemos%2Flakehouse%2Flakehouse-fsi-fraud%2F01-Data-ingestion%2F01.1-DLT-fraud-detection-SQL&version=1">
...
-- MAGIC %md
-- MAGIC Your DLT Pipeline has been installed and started for you! Open the <a dbdemos-pipeline-id="dlt-fsi-fraud" href="#joblist/pipelines/2f344780-5b93-439b-ae7a-771f301249d2" target="_blank">Fraud detection Delta Live Table pipeline</a> to see it in action.<br/>
...
-- MAGIC %python
-- MAGIC display(spark.read.json('/Volumes/tddbricks_sea/dbdemos_fsi_fraud_detection/fraud_raw_data/transactions'))

-- COMMAND ----------

-- DBTITLE 1,Raw incoming customers (json)
-- MAGIC %python
-- MAGIC display(spark.read.csv('/Volumes/tddbricks_sea/dbdemos_fsi_fraud_detection/fraud_raw_data/customers', header=True, multiLine=True))

-- COMMAND ----------

-- DBTITLE 1,Raw incoming country
-- MAGIC %python
-- MAGIC display(spark.read.csv('/Volumes/tddbricks_sea/dbdemos_fsi_fraud_detection/fraud_raw_data/country_code', header=True))
...
AS 
  SELECT * FROM cloud_files("/Volumes/tddbricks_sea/dbdemos_fsi_fraud_detection/fraud_raw_data/transactions", "json", map("cloudFiles.maxFilesPerTrigger", "1", "cloudFiles.inferColumnTypes", "true"))
...
AS 
  SELECT * FROM cloud_files("/Volumes/tddbricks_sea/dbdemos_fsi_fraud_detection/fraud_raw_data/customers", "csv", map("cloudFiles.inferColumnTypes", "true", "multiLine", "true"))
...
AS 
  SELECT * FROM cloud_files("/Volumes/tddbricks_sea/dbdemos_fsi_fraud_detection/fraud_raw_data/country_code", "csv")
...
AS 
  SELECT * FROM cloud_files("/Volumes/tddbricks_sea/dbdemos_fsi_fraud_detection/fraud_raw_data/fraud_report", "csv")
...
-- MAGIC
-- MAGIC Open the <a dbdemos-pipeline-id="dlt-fsi-fraud" href="#joblist/pipelines/2f344780-5b93-439b-ae7a-771f301249d2" target="_blank">Fraud detection Delta Live Table pipeline</a> and click on start to visualize your lineage and consume the new data incrementally!
