...
# MAGIC %sql
# MAGIC select * from tddbricks_sea.trishaldoshi_aml.transactions
...
# MAGIC %sql
# MAGIC   select * 
# MAGIC   from tddbricks_sea.trishaldoshi_aml.entities
# MAGIC   where email_addr in 
# MAGIC   (
# MAGIC     select A.email_addr 
# MAGIC     from 
# MAGIC       (
# MAGIC         select email_addr, count(*) as cnt 
# MAGIC         from tddbricks_sea.trishaldoshi_aml.entities
# MAGIC         group by email_addr
# MAGIC       ) A
# MAGIC     where A.cnt > 1
# MAGIC   )
# MAGIC   order by email_addr
# MAGIC
...
identity_edges = sql('''
select entity_id as src, address as dst from tddbricks_sea.trishaldoshi_aml.entities where address is not null
union
select entity_id as src, email_addr as dst from tddbricks_sea.trishaldoshi_aml.entities where email_addr is not null
union
select entity_id as src, phone_number as dst from tddbricks_sea.trishaldoshi_aml.entities where phone_number is not null
''')

identity_nodes = sql('''
select distinct(entity_id) as id, 'Person' as type from tddbricks_sea.trishaldoshi_aml.entities
union 
select distinct(address) as id, 'Address' as type from tddbricks_sea.trishaldoshi_aml.entities
union 
select distinct(email_addr) as id, 'Email' as type from tddbricks_sea.trishaldoshi_aml.entities
union 
select distinct(phone_number) as id, 'Phone' as type from tddbricks_sea.trishaldoshi_aml.entities
''')
...
# DBTITLE 1,Identifying Duplicate Person Components
# MAGIC %sql
...
# DBTITLE 1,All Synthetic IDs from ptntl_synthetic_ids
# MAGIC %sql
# MAGIC select * from ptntl_synthetic_ids

# COMMAND ----------

# DBTITLE 1,Identifying Components from Synthetic IDs
# MAGIC %sql
...
# DBTITLE 1,Component Analysis for Specific ID
# MAGIC %sql
...
# DBTITLE 1,GraphFrame for AML Transactions
...
from tddbricks_sea.trishaldoshi_aml.transactions
""")
...
levels = sql(
    ...
    from tddbricks_sea.trishaldoshi_aml.structuring graph
    join tddbricks_sea.trishaldoshi_aml.entities entity0
    on graph.a.id = entity0.entity_id
    join tddbricks_sea.trishaldoshi_aml.entities entity1
    on graph.b.id = entity1.entity_id
    join tddbricks_sea.trishaldoshi_aml.entities entity2 
    on graph.c.id = entity2.entity_id
    join tddbricks_sea.trishaldoshi_aml.entities entity3
...
display(
  sql(
    ...
    from
      tddbricks_sea.trishaldoshi_aml.roundtrips rt
      join tddbricks_sea.trishaldoshi_aml.entities ents0 on rt.a.id = ents0.entity_id
      join tddbricks_sea.trishaldoshi_aml.entities ents1 on rt.b.id = ents1.entity_id
      join tddbricks_sea.trishaldoshi_aml.entities ents2 on rt.c.id = ents2.entity_id
      join tddbricks_sea.trishaldoshi_aml.entities ents3 on rt.d.id = ents3.entity_id
    """)
  )
...
from
  (
    select 
      ...
      risk_score risk 
    from tddbricks_sea.trishaldoshi_aml.entities
  ) A
""")
...
display(
  sql(
    ...
    select
      ...
      b.name
    from
      tddbricks_sea.trishaldoshi_aml.risk_propagation a
      join tddbricks_sea.trishaldoshi_aml.entities b on a.id = b.entity_id
    where
      id >= 10000001
    """
  )
